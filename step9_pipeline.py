# %% [markdown]
# # Step 9 — Coordination z-scores + evidence chronology for *Detect Suspicious Value Transfers in Poker*
#
...
WORK_DIR = Path("/kaggle/working/step9") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof,
              **{f"oof_{fam}": oof_fam[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "dev_oof_v9.parquet", index=False)
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v9.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v9.parquet", index=False)
