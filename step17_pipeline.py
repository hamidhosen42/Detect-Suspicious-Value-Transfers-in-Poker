# %% [markdown]
# # Step 17 — Phase-robust multiple-instance hand model for *Detect Suspicious Value Transfers in Poker*
#
# Step 16 (Step 8 pair features + upgraded evidence re-ranker) is the base. **New in Step 17: the MIL hand model, third version — without the within-pair
# percentile / max-normalised hand features.** Steps 11–15 learned "hand of a coordinated pair vs. hand of an ordinary pair" mostly from `_prank` / `_to_max`
# features, which are percentiles over all shared hands of the pair in its phase: ≈ 110 hands in the 3,000-hand development phase but ≈ 76 in the 2,000-hand
# evaluation phase. Ordinary evaluation hands therefore looked more extreme than any development negative the pair model had seen (MIL tails 2.5× higher even
# when every evaluation hand was scored by the fold model that never saw its table), and the leaderboard fell from 0.894 to 0.872. With those 124 features
# removed the development and evaluation MIL distributions agree (negatives' 99th percentile of the top-3 mean: 0.22 vs 0.27; mean and counts identical), and
# the three development views all improve on the Step 8 pair features: labelled AP 0.988 → 0.994, pessimistic AP 0.689 → 0.715, hidden-150 AP 0.924 → 0.932.
# Only the three least exposure-sensitive aggregates are used (top-3 mean, mean, max); evaluation hands are scored leave-table-out.
#
# ---
#
...
WORK_DIR = Path("/kaggle/working/step17") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
# %% [markdown]
# ## 6b. Phase-robust multiple-instance hand model (Step 17)

# %%
MIL_DEV = [WORK_DIR / f"mil_dev_w{w}_v17.parquet" for w in range(len(TRAIN_WINDOWS))]
MIL_EVAL = WORK_DIR / "mil_eval_v17.parquet"
MIL_COLS = ["mil_top3", "mil_mean", "mil_max"]
MIL_FEATS = [c for c in HAND_FEATS if not c.endswith("_prank") and not c.endswith("_to_max")]   # no within-pair percentiles: phase-length independent
...
if not (all(p.exists() for p in MIL_DEV) and MIL_EVAL.exists()):
    ...
    log(f"MIL negative selection: quick pair OOF labelled AP={average_precision_score(_y0[_lab0 & (dev_pf['window'] == 0).to_numpy()], _o0[_lab0 & (dev_pf['window'] == 0).to_numpy()]):.4f}")
    ...
    _hs = pl.concat([pl.scan_parquet(f).filter(pl.col("pair_id").is_in(list(_keep))).select(["pair_id", "hand_id", "table_id", "hand_idx", *MIL_FEATS]).collect() for f in DEV_HF])
    ...
    log(f"MIL hand sample {_hs.shape}; hands of coordinated pairs {(_hs['plabel'] == 1).sum():,}; bags {_hs['bag'].n_unique():,}; {len(MIL_FEATS)} phase-robust hand features")
    _X = _hs.select(MIL_FEATS).to_numpy().astype(np.float32); _pl = _hs["plabel"].to_numpy(); _fold = _hs["fold"].to_numpy(); _bag = _hs["bag"].to_numpy()
    ...
    for it in range(MIL_ITERS):
        ...
        log(f"MIL round {it + 1}: bag AP from the top-3 mean hand score (all sampled bags) = {average_precision_score(_agg['pl'], _agg['top3']):.4f}")
    ...
    def mil_aggregate(lf: pl.LazyFrame) -> pl.DataFrame:
        """Leave-table-out MIL score for every hand (development and evaluation alike), aggregated per pair."""
        parts = []; n = lf.select(pl.len()).collect().item()
        for st in range(0, n, 1_500_000):
            c = lf.slice(st, 1_500_000).collect(); Xc = c.select(MIL_FEATS).to_numpy().astype(np.float32); sc = np.zeros(c.height)
        ...
        return pl.concat(parts).group_by("pair_id").agg(pl.col("mil").top_k(3).mean().alias("mil_top3"), pl.col("mil").mean().alias("mil_mean"), pl.col("mil").max().alias("mil_max"))

    for w, (lo, hi) in enumerate(TRAIN_WINDOWS):
        mil_aggregate(pl.concat([pl.scan_parquet(f) for f in DEV_HF]).filter((pl.col("hand_idx") >= lo) & (pl.col("hand_idx") < hi))).write_parquet(MIL_DEV[w])
        log(f"MIL aggregates for development window {w} written")
    mil_aggregate(pl.scan_parquet(EVAL_HF)).write_parquet(MIL_EVAL)
    log("MIL aggregates for evaluation written")
    del mil_models; gc.collect()
_mil = [pd.read_parquet(p).set_index("pair_id") for p in MIL_DEV]
_vals = np.stack([_mil[w].reindex(dev_pf["pair_id_orig"]).to_numpy() for w in range(len(TRAIN_WINDOWS))])
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof}).to_parquet(WORK_DIR / "dev_oof_v17.parquet", index=False)
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v17.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v17.parquet", index=False)
