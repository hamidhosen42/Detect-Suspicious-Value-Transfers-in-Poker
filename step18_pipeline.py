# %% [markdown]
# # Step 18 — Evidence ranker with made-hand combinations, street timing and per-player policy surprise
#
# Step 16 (LB 0.89625; blend with Step 9 0.89716) is the base; the pair model is untouched (risk identical to Step 8/16).
# **New in Step 18 — a separate evidence stage-1 ranker** with three feature families validated on the development pool with two fold permutations
# and three seeds (stage-1 MAP@5 +0.009 / +0.011): (1) `cc_*` made-hand categories of both partners at the decisive street, who folded, whether
# the loser held the better hand, outsiders' categories and folds; (2) `st_*` street timing of the partner interaction (first / last street facing
# each other, streets both stayed in, heads-up streets, outsiders left on the decisive street, first deviating street); (3) `pp_*` action surprise
# under a **per-player** policy fitted on the player's own other-phase decisions (within-pair percentile versions excluded: phase-length dependent).
# The Step 8 hand score still feeds the pair aggregates; the new ranker only selects and orders the evidence pool for stage 2.
#
# ---
#
# # Step 16 — Step 8 pair model + upgraded evidence re-ranker
#
# **Step 8 scored 0.89378** and remains the best leaderboard pair ranking (Step 9's coordination z-scores were neutral: 0.89174; the MIL hand model of
# Steps 11–15 did not transfer from the 3,000-hand development phase to the 2,000-hand evaluation phase and is dropped). Step 16 keeps the Step 8 pair
# features and model untouched and upgrades only the **evidence stage-2 re-ranker** with the changes validated on the development pool:
# the pair's family (true in training, predicted at inference), the chronology of strong hands in the pool, and smoothed target encodings of the pair's
# ordered action-sequence template (dev pool MAP@5 0.578 → 0.609).
#
# ---
#
...
WORK_DIR = Path("/kaggle/working/step18") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
BASELINE_COLS = ["net_bb", "vpip", "n_aggr", "n_raise", "n_call", "n_fold_facing", "went_to_showdown", "contrib_bb", "vpip_a", "loose", "junk_vpip", "vpip_resid", "pfr", "surp_sum", "surp_max", "surp_post", "surp_pf", "surp_fold", "surp_call"]
baselines = (
...
DEV_HF = [WORK_DIR / f"dev_hand_features_v8_{k}.parquet" for k in range(N_TABLE_CHUNKS)]
EVAL_HF = WORK_DIR / "eval_hand_features_v8.parquet"
...
# --- Step 18: evidence-only stage-1 ranker with the hypothesis feature families ------------------------------------------
HYP = WORK_DIR / "hyp"
_cc = pl.read_parquet(HYP / "category_combo.parquet"); CC_FEATS = [c for c in _cc.columns if c.startswith("cc_") and "_te" not in c]
_st = pl.read_parquet(HYP / "street_timing.parquet"); ST_FEATS = [c for c in _st.columns if c not in ("pair_id", "hand_id")]
_pp = pl.read_parquet(HYP / "player_policy.parquet"); PP_FEATS = [c for c in _pp.columns if c not in ("pair_id", "hand_id") and not c.endswith("_prank")]
HYP_FEATS = CC_FEATS + ST_FEATS + PP_FEATS
EV_FEATS = HAND_FEATS + HYP_FEATS
_hyp_dev = _cc.select(["pair_id", "hand_id", *CC_FEATS]).join(_st.select(["pair_id", "hand_id", *ST_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True).join(_pp.select(["pair_id", "hand_id", *PP_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True).to_pandas()
dh = dh.merge(_hyp_dev, on=["pair_id", "hand_id"], how="left")
dh[HYP_FEATS] = dh[HYP_FEATS].astype("float32").fillna(0.0)
log(f"hypothesis features joined: {len(CC_FEATS)} cc + {len(ST_FEATS)} st + {len(PP_FEATS)} pp; coverage on coordinated pairs' hands {dh.loc[pos_mask, CC_FEATS[0]].notna().mean():.3f}")
dh["hand_score_ev"] = 0.0
rank_models_ev = {}
for f in range(N_FOLDS):
    tr_pos = pos_mask & (dh["fold"] != f).to_numpy(); te = (dh["fold"] == f).to_numpy()
    rank_models_ev[f] = [lgb.train({**HAND_PARAMS, "seed": sd}, lgb.Dataset(dh.loc[tr_pos, EV_FEATS], dh.loc[tr_pos, "is_evidence"].astype(int)), num_boost_round=HAND_ROUNDS) for sd in HAND_SEEDS]
    dh.loc[te, "hand_score_ev"] = np.mean([m.predict(dh.loc[te, EV_FEATS]) for m in rank_models_ev[f]], axis=0)
rank_models_ev_full = [lgb.train({**HAND_PARAMS, "seed": sd}, lgb.Dataset(dh.loc[pos_mask, EV_FEATS], dh.loc[pos_mask, "is_evidence"].astype(int)), num_boost_round=HAND_ROUNDS) for sd in HAND_SEEDS]
log(f"evidence stage-1 with hypothesis features: OOF MAP@5 {map5_within_pairs(dh[pos_mask]):.4f} -> {map5_within_pairs(dh[pos_mask], score_col='hand_score_ev'):.4f}")
imp_ev = pd.Series(np.mean([m.feature_importance("gain") for m in rank_models_ev_full], axis=0), index=EV_FEATS).sort_values(ascending=False)
print("top evidence-ranker (v18) features:\n", imp_ev.head(20).round(0).to_string())
...
# --- Step 16: action-sequence templates of the pair (winner / loser roles, street, facing whom, action) ---------------------------------------
...
POOL_FEATS = EV_FEATS + POOL_EXTRA
...
dh_pos = pl.from_pandas(dh.loc[pos_mask, ["pair_id", "hand_id", "hand_idx", "hand_score_ev", "is_evidence", "fold", "pot_bb", "behavior_family", *TE_FEATS] + [c for c in EV_FEATS if c != "pot_bb"]]).rename({"hand_score_ev": "hand_score"})
...
dh["hand_final"] = np.where(dh["hand_score2"].notna(), 10.0 + dh["hand_score2"], dh["hand_score_ev"])
pos_all = dh[pos_mask]
...
AGG_SUM = ["fold_better_to_partner", "dump_better_hand", "squeeze_then_fold_pf", "loser_beats_winner_final", "both_made_no_aggr",
           ...
           "surp_vs_partner", "surp_fold_partner", "surp_call_partner", "both_surp2", "n_surp2_sum"]
...
DEV_PF = WORK_DIR / "dev_pair_features_v8.parquet"
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof}).to_parquet(WORK_DIR / "dev_oof_v18.parquet", index=False)
...
# %%
EVAL_SCORED = WORK_DIR / "eval_hand_scored_v8.parquet"   # Step 8 hand scores are reused
...
# Step 18: evidence-only score for every evaluation hand (hypothesis features computed for the evaluation phase)
HYP_EVAL_SCORED = WORK_DIR / "eval_hand_score_ev_v18.parquet"
if not HYP_EVAL_SCORED.exists():
    _hyp_e = (pl.scan_parquet(HYP / "eval" / "category_combo_eval.parquet").select(["pair_id", "hand_id", *CC_FEATS])
              .join(pl.scan_parquet(HYP / "eval" / "street_timing_eval.parquet").select(["pair_id", "hand_id", *ST_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True)
              .join(pl.scan_parquet(HYP / "eval" / "player_policy_eval.parquet").select(["pair_id", "hand_id", *PP_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True).collect())
    log(f"evaluation hypothesis features: {_hyp_e.shape}")
    parts = []; n = eh.height
    for start in range(0, n, 1_500_000):
        c = eh.slice(start, 1_500_000).select(["pair_id", "hand_id", *HAND_FEATS]).join(_hyp_e, on=["pair_id", "hand_id"], how="left")
        Xc = c.select(EV_FEATS).to_pandas().astype("float32").fillna(0.0)
        parts.append(c.select(["pair_id", "hand_id"]).with_columns(pl.Series("hand_score_ev", np.mean([m.predict(Xc) for m in rank_models_ev_full], axis=0), dtype=pl.Float32)))
        log(f"scored eval hands (v18 evidence ranker) {min(start + 1_500_000, n):,}/{n:,}")
    pl.concat(parts).write_parquet(HYP_EVAL_SCORED); del parts, c, Xc; gc.collect()
eh = eh.join(pl.read_parquet(HYP_EVAL_SCORED), on=["pair_id", "hand_id"], how="left").with_columns(pl.col("hand_score_ev").fill_null(0.0))
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v18.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v18.parquet", index=False)
...
_hyp_e = (pl.scan_parquet(HYP / "eval" / "category_combo_eval.parquet").select(["pair_id", "hand_id", *CC_FEATS])
          .join(pl.scan_parquet(HYP / "eval" / "street_timing_eval.parquet").select(["pair_id", "hand_id", *ST_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True)
          .join(pl.scan_parquet(HYP / "eval" / "player_policy_eval.parquet").select(["pair_id", "hand_id", *PP_FEATS]), on=["pair_id", "hand_id"], how="full", coalesce=True))
pool_eval = pool_features(eh.select(["pair_id", "hand_id", "hand_idx", pl.col("hand_score_ev").alias("hand_score"), "pot_bb"] + [c for c in HAND_FEATS if c != "pot_bb"]).join(eval_fam, on="pair_id", how="left")
                          .with_columns([(pl.col("_fam") == f).cast(pl.Float32).alias(f"fam_{f}") for f in TARGET_BEHAVIORS]).drop("_fam"))
pool_eval = pool_eval.join(_hyp_e.collect(), on=["pair_id", "hand_id"], how="left").with_columns([pl.col(c).fill_null(0.0).cast(pl.Float32) for c in HYP_FEATS])
...
eh = eh.join(pool_eval.select(["pair_id", "hand_id", "hand_score2"]), on=["pair_id", "hand_id"], how="left").with_columns(
    pl.when(pl.col("hand_score2").is_not_null()).then(10.0 + pl.col("hand_score2")).otherwise(pl.col("hand_score_ev")).cast(pl.Float32).alias("hand_final"))
