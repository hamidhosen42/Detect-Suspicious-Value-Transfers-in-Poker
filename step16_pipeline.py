# %% [markdown]
# # Step 16 — Step 8 pair model + upgraded evidence re-ranker for *Detect Suspicious Value Transfers in Poker*
#
...
WORK_DIR = Path("/kaggle/working/step16") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof}).to_parquet(WORK_DIR / "dev_oof_v16.parquet", index=False)
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v16.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v16.parquet", index=False)
