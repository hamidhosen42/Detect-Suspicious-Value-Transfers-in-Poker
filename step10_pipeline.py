# %% [markdown]
# # Step 10 — Transductive evaluation-phase PU for *Detect Suspicious Value Transfers in Poker*
#
# **Step 8 scored 0.89378**; Step 9 (z-scores, family-aware evidence) is its base here.
#
# **New in Step 10 — the two-step PU is applied across phases.** The evaluation pairs are unlabelled data from the same generator; after the development
# model has scored them, the ones that score like confirmed colluders become pseudo-positives and the rest weak negatives, and the pair ranker is retrained on
# development windows **and** evaluation pairs together (table-grouped folds shared by both phases, so the development OOF stays honest and every evaluation
# pair is scored by models that never saw it or its table). Two rounds. Motivation: on the evaluation-like development population (≥ 38 shared hands) the
# labelled AP is already 0.996, so the remaining leaderboard loss sits at the colluder / loose-bot boundary of the evaluation population itself — which the
# transductive round fits directly (evaluation exposure = 2,000 hands, evaluation-phase field baselines).
# Ablation kept: rows with < 38 shared hands in a training window stay in training (dropping or down-weighting them lowers every development view).
#
# ---
#
...
WORK_DIR = Path("/kaggle/working/step10") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
dh_pos = pl.from_pandas(dh.loc[pos_mask, ["pair_id", "hand_id", "hand_idx", "hand_score", "is_evidence", "fold", "pot_bb", "behavior_family"] + [c for c in HAND_FEATS if c != "pot_bb"]])
...
DEV_PF = WORK_DIR / "dev_pair_features_v9.parquet"
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof,
              **{f"oof_{fam}": oof_fam[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "dev_oof_v10.parquet", index=False)
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v10.parquet", index=False)
...
risk_dev_only = risk.copy()

# %% [markdown]
# ## 7b. Transductive PU on the evaluation pairs (Step 10)

# %%
TRANS_ROUNDS = 2
EVAL_PSEUDO_WEIGHT, EVAL_AMBIG_WEIGHT, EVAL_NEG_WEIGHT = 1.0, 0.2, 0.1
eval_fold = eval_pf["table_id"].map(TABLE_FOLD).to_numpy()
X_all = pd.concat([X, Xe], ignore_index=True)
folds_all = np.concatenate([folds, eval_fold])
n_dev = len(X)
pos_oof_ref = oof[labm & (y == 1)]
thr_amb_e, thr_ps_e = np.quantile(pos_oof_ref, 0.05), np.quantile(pos_oof_ref, 0.25)
risk_t = risk.copy()
for rnd in range(TRANS_ROUNDS):
    e_pseudo = risk_t >= thr_ps_e
    e_amb = (risk_t >= thr_amb_e) & ~e_pseudo
    y_all = np.concatenate([y2, e_pseudo.astype(int)])
    w_all = np.concatenate([w_train2, np.where(e_pseudo, EVAL_PSEUDO_WEIGHT, np.where(e_amb, EVAL_AMBIG_WEIGHT, EVAL_NEG_WEIGHT))])
    print(f"transductive round {rnd + 1}: evaluation pseudo-positives {int(e_pseudo.sum())}, ambiguous {int(e_amb.sum())}, weak negatives {int((~e_pseudo & ~e_amb).sum())}")
    oof_all = np.zeros(len(y_all)); trans_models = []
    for f in range(N_FOLDS):
        tr, te = folds_all != f, folds_all == f
        for sd in PAIR_SEEDS + [SEED + 37, SEED + 51]:
            m = lgb.train({**PAIR_PARAMS, "seed": sd}, lgb.Dataset(X_all[tr], y_all[tr], weight=w_all[tr]), num_boost_round=PAIR_ROUNDS)
            oof_all[te] += m.predict(X_all[te]) / 5
            trans_models.append(m)
    oof_t, risk_t = oof_all[:n_dev], oof_all[n_dev:]
    W0m = W0
    print(f"  dev views after round {rnd + 1}: labelled AP={average_precision_score(y[labm], oof_t[labm]):.4f} | pessimistic AP={average_precision_score(y, oof_t):.4f} | "
          f"labelled AP on n_shared>=38 (window 0)={average_precision_score(y[labm & W0m & (dev_pf['n_shared'].to_numpy() >= 38)], oof_t[labm & W0m & (dev_pf['n_shared'].to_numpy() >= 38)]):.4f} | "
          f"pessimistic on n_shared>=38 (window 0)={average_precision_score(y[W0m & (dev_pf['n_shared'].to_numpy() >= 38)], oof_t[W0m & (dev_pf['n_shared'].to_numpy() >= 38)]):.4f}")
    print(f"  eval: pairs with risk>0.9 {int((risk_t > 0.9).sum())}, >0.5 {int((risk_t > 0.5).sum())}; spearman vs dev-only model {pd.Series(risk_t).corr(pd.Series(risk_dev_only), method='spearman'):.4f}")
print(f"dev-only model views for reference: labelled AP={labelled_ap:.4f} | pessimistic AP={pu_ap:.4f}")
risk = risk_t          # evaluation risk = OOF of the transductive models (each pair scored without its own table)
oof_trans = oof_t
eval_pf["risk_score"] = risk
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, "risk_dev_only": risk_dev_only, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v10.parquet", index=False)
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof_trans}).to_parquet(WORK_DIR / "dev_oof_v10_trans.parquet", index=False)
