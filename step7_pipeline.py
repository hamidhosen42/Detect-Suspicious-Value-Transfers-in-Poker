# %% [markdown]
# # Step 7 — Both-phase player baselines for *Detect Suspicious Value Transfers in Poker*
#
# **Step 6 scored 0.84157** (Step 5: 0.82369, Step 4: 0.81982, Step 3: 0.80136).
#
# **New in Step 7.** The strongest pair feature is the partner-vs-field contrast of the policy surprise (`loose_pf_sum`): how a player plays with the partner minus
# how the same player plays in all other hands. The "field" was estimated from the same phase only (≤ 2,000 hands on evaluation). Because collusion is phase-specific
# (verified in Step 5), a player's hands in the *other* phase — including hands with the partner — are honest baseline too. The field now uses **both phases**
# (up to 5,000 hands per player), which makes every contrast feature sharper exactly where evaluation exposure is short. A third 2,000-hand training window is added.
#
# ---
#
...
WORK_DIR = Path("/kaggle/working/step7") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step7"))
...
BASELINE_COLS = ["net_bb", "vpip", "n_aggr", "n_raise", "n_call", "n_fold_facing", "went_to_showdown", "contrib_bb", "vpip_a", "loose", "junk_vpip", "vpip_resid", "pfr"]
baselines = (
...
# Step 7: the field baseline uses both phases (collusion is phase-specific, so the other phase is honest baseline for every pair)
baselines_both = (
    pl.scan_parquet(PLAYER_HANDS)
    .group_by("player_id")
    .agg(pl.len().alias("base_hands"), *[pl.col(c).cast(pl.Float32).mean().alias(f"base_{c}") for c in BASELINE_COLS])
...
# %%
PAIR_PHASE = WORK_DIR / "pair_other_phase.parquet"
OB_STATS = ["both_vpip", "both_junk", "one_junk", "loose_sum", "loose_min", "resid_sum", "junk_vpip_sum", "junk_pfr_sum", "transfer", "n_vpip", "both_fold"]
if not PAIR_PHASE.exists():
    ...
    ph_ = pl.scan_parquet(PLAYER_HANDS).select(["hand_id", "player_id", "phase", "vpip_a", "pfr", "chen", "loose", "vpip_resid", "junk_vpip", "junk_pfr", "net_bb", "folded"])
    (
...
P_COLS = ["seat_no", "contrib_bb", "net_bb", "vpip", "folded", "went_to_showdown", "won_share", "chen",
          "n_actions", "n_aggr", "n_raise", "n_call", "n_check", "n_allin", "n_fold_facing", "n_postflop_check", "n_hu_actions", "last_street", "max_amount_bb", "max_to_call_bb",
          "pos", "vpip_a", "pfr", "pf_entry_no", "pf_faced_raise", "p_vpip", "loose", "tight", "loose_pfr", "junk_vpip", "junk_pfr", "vpip_resid"]
...
TRAIN_WINDOWS = [(0, N_EVAL_HANDS), ((N_DEV_HANDS - N_EVAL_HANDS) // 2, (N_DEV_HANDS + N_EVAL_HANDS) // 2), (N_DEV_HANDS - N_EVAL_HANDS, N_DEV_HANDS)]   # 2,000-hand development windows = evaluation exposure
...
dh_pos = pl.from_pandas(dh.loc[pos_mask, ["pair_id", "hand_id", "hand_idx", "hand_score", "is_evidence", "fold", "pot_bb"] + [c for c in HAND_FEATS if c != "pot_bb"]])
...
CONTRAST = ["net_bb", "n_aggr", "n_raise", "n_call", "n_fold_facing", "vpip", "vpip_a", "loose", "junk_vpip", "vpip_resid", "pfr"]
...
def aggregate_pairs(hf: pl.DataFrame, baselines_phase: pl.DataFrame, pairs: pl.DataFrame) -> pd.DataFrame:
    ...
    pf = pf.with_columns((pl.col("ob_n") / (pl.col("ob_n") + OB_K)).alias("_ob_w")).with_columns(
        ...
        *[((pl.col(f"_ip_{c}") + 0.02) / (pl.col("_ob_w") * pl.col(f"ob_{c}") + (1 - pl.col("_ob_w")) * OB_MEDIAN[c] + 0.02)).cast(pl.Float32).alias(f"{c}_ratio_other") for c in ["both_vpip", "both_junk", "loose_sum", "junk_vpip_sum", "transfer"]],
    )
...
base_dev = baselines_both
DEV_PF = WORK_DIR / "dev_pair_features_v7.parquet"
...
w_train2 = np.where(labm, np.where(y == 1, POS_WEIGHT, NEG_WEIGHT), np.where(pseudo, PSEUDO_POS_WEIGHT, np.where(ambiguous, 0.0, PU_WEIGHT)))
...
pool_eval = pool_features(eh.select(["pair_id", "hand_id", "hand_idx", "hand_score", "pot_bb"] + [c for c in HAND_FEATS if c != "pot_bb"]))
