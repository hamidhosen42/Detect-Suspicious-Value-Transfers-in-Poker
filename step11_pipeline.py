# %% [markdown]
# # Step 11 — Multiple-instance hand model for *Detect Suspicious Value Transfers in Poker*
#
...
WORK_DIR = Path("/kaggle/working/step11") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
# %% [markdown]
# ## 5c. Multiple-instance hand model (Step 11): hand of a coordinated pair vs. hand of an ordinary pair

# %%
MIL_DEV = [WORK_DIR / f"mil_dev_w{w}_v11.parquet" for w in range(len(TRAIN_WINDOWS))]
MIL_EVAL = WORK_DIR / "mil_eval_v11.parquet"
...
MIL_ROUNDS, MIL_K, MIL_ITERS, MIL_NEG_PAIRS = 300, 8, 3, 6000
if not (all(p.exists() for p in MIL_DEV) and MIL_EVAL.exists()):
    _lab = dict(zip(dev_pairs["pair_id"], dev_pairs["label"].fill_null(0))); _islab = dict(zip(dev_pairs["pair_id"], dev_pairs["is_labeled"]))
    _rs = np.random.RandomState(SEED + 1)
    _unl = [p for p in dev_pairs["pair_id"] if not _islab[p]]
    _keep = set(p for p in dev_pairs["pair_id"] if _islab[p]) | set(_rs.choice(_unl, MIL_NEG_PAIRS, replace=False))
    _hs = pl.concat([pl.scan_parquet(f).filter(pl.col("pair_id").is_in(list(_keep)) & (pl.col("hand_idx") < N_EVAL_HANDS)).select(["pair_id", "hand_id", "table_id", "hand_idx", *HAND_FEATS]).collect() for f in DEV_HF])
    ...
    log(f"MIL hand sample {_hs.shape}; hands of coordinated pairs {(_hs['plabel'] == 1).sum():,}")
    _X = _hs.select(HAND_FEATS).to_numpy().astype(np.float32); _pl = _hs["plabel"].to_numpy(); _fold = _hs["fold"].to_numpy(); _pid = _hs["pair_id"].to_numpy()
    ...
    for it in range(MIL_ITERS):
        ...
        _df = pd.DataFrame({"pid": _pid, "s": _s, "pl": _pl}); _df["rk"] = _df.groupby("pid")["s"].rank(ascending=False, method="first")
        _y = np.where((_df["pl"] == 1) & (_df["rk"] <= MIL_K), 1, 0); _w = np.where(_df["pl"] == 1, np.where(_df["rk"] <= MIL_K, 1.0, 0.2), 1.0)
        _agg = _df.groupby("pid").agg(top3=("s", lambda g: g.nlargest(3).mean()), pl=("pl", "first"))
        _la = _agg[[_islab[p] for p in _agg.index]]
        log(f"MIL round {it + 1}: labelled-pair AP from the top-3 mean hand score = {average_precision_score(_la['pl'], _la['top3']):.4f}")
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof,
              **{f"oof_{fam}": oof_fam[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "dev_oof_v11.parquet", index=False)
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v11.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v11.parquet", index=False)
