# %% [markdown]
# # Step 14 — Leave-table-out scoring of the evaluation phase for *Detect Suspicious Value Transfers in Poker*
#
# **Step 12 scored 0.87283 on the leaderboard — below Step 8 (0.89378) although every development view was far better.** The cause is a stacking
# regime mismatch: development pair features built from stacked hand models (MIL, evidence ranker) are out-of-fold by table, but the evaluation hands were
# scored with the *full* models trained on all 400 tables. Those are sharper: evaluation negatives' MIL aggregates sit ~3× higher in the tail than the
# development negatives the pair model was calibrated on (mil_max 99th pct 0.149 vs 0.058; mil_top3 0.096 vs 0.032), so ordinary evaluation pairs land
# where only colluders sat in development. Raw features (surprise sums, z-scores) show no such shift. **Step 14 scores every stacked quantity for an
# evaluation pair with the fold model that never saw that pair's table** — the evidence ranker, the suspicious-hand model, the MIL model, the stage-2 pool
# re-ranker and the template target encodings — so development and evaluation share one regime.
#
# ---
#
# # Step 13 — MIL v2
...
WORK_DIR = Path("/kaggle/working/step14") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step6"))
...
TE_MAPS_F = {f: {c: (_tk[c][pos_mask & (_fo != f)], _yev[pos_mask & (_fo != f)]) for c in TPL_COLS} for f in range(N_FOLDS)}
TE_MAPS_FAM_F = {f: {c: (_tkf[c][pos_mask & (_fo != f)], _yev[pos_mask & (_fo != f)]) for c in TPL_COLS} for f in range(N_FOLDS)}
...
# %%
MIL_DEV = [WORK_DIR / f"mil_dev_w{w}_v14.parquet" for w in range(len(TRAIN_WINDOWS))]
MIL_EVAL = WORK_DIR / "mil_eval_v14.parquet"
...
if not (all(p.exists() for p in MIL_DEV) and MIL_EVAL.exists()):
    ...
    mil_aggregate(pl.scan_parquet(EVAL_HF), True).write_parquet(MIL_EVAL)   # step 14: leave-table-out for evaluation too
...
pd.DataFrame({"pair_id": dev_pf["pair_id"], "pair_id_orig": dev_pf["pair_id_orig"], "window": dev_pf["window"], "label": y, "is_labeled": labm, "oof": oof,
              **{f"oof_{fam}": oof_fam[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "dev_oof_v14.parquet", index=False)
...
# %%
EVAL_SCORED = WORK_DIR / "eval_hand_scored_v14.parquet"
if not EVAL_SCORED.exists():
    ...
    for start in range(0, n, CH):
        ...
        fo = chunk["table_id"].replace_strict(TABLE_FOLD, return_dtype=pl.Int8).to_numpy()
        s1 = np.zeros(chunk.height, np.float32); s2 = np.zeros(chunk.height, np.float32)
        for f in range(N_FOLDS):   # step 14: each evaluation hand is scored by the models that never saw its table
            m = fo == f
            if m.any():
                s1[m] = np.mean([mm.predict(Xc[m]) for mm in rank_models[f]], axis=0)
                s2[m] = sus_models[f].predict(Xc[m])
...
eval_pf.to_parquet(WORK_DIR / "eval_pair_features_v14.parquet", index=False)
...
pd.DataFrame({"pair_id": eval_pf["pair_id"], "risk": risk, **{f"p_{fam}": fam_probs[fam] for fam in TARGET_BEHAVIORS}}).to_parquet(WORK_DIR / "eval_risk_v14.parquet", index=False)
...
_te_e = {}
_tpl_e = _tpl_e.join(eh.select(["pair_id", "table_id"]).unique(), on="pair_id", how="left")
_fo_e = _tpl_e["table_id"].replace_strict(TABLE_FOLD, return_dtype=pl.Int8).to_numpy()
for c in TPL_COLS:
    _k1 = _tpl_e[c].to_numpy(); _k2 = (_tpl_e["_fam"].cast(pl.Utf8) + "|" + _tpl_e[c]).to_numpy()
    _v1 = np.zeros(len(_k1)); _v2 = np.zeros(len(_k1))
    for f in range(N_FOLDS):   # step 14: encode with the maps that exclude this table's fold
        m = _fo_e == f
        if m.any():
            _v1[m] = te_encode(*TE_MAPS_F[f][c], _k1[m]); _v2[m] = te_encode(*TE_MAPS_FAM_F[f][c], _k2[m])
    _te_e[f"te_{c}"] = _v1; _te_e[f"te_{c}_fam"] = _v2
...
_Xp = pool_eval.select(POOL_FEATS).to_pandas().astype("float32"); _fo_p = pool_eval.join(eh.select(["pair_id", "table_id"]).unique(), on="pair_id", how="left")["table_id"].replace_strict(TABLE_FOLD, return_dtype=pl.Int8).to_numpy()
s2 = np.zeros(pool_eval.height)
for f in range(N_FOLDS):   # step 14: leave-table-out stage-2 scores
    m = _fo_p == f
    if m.any(): s2[m] = np.mean([mm.predict(_Xp[m]) for mm in pool_models[f]], axis=0)
