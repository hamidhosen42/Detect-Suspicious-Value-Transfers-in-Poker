# %% [markdown]
# # Step 3 — Card-conditional policy deviation for *Detect Suspicious Value Transfers in Poker*
#
...
WORK_DIR = Path("/kaggle/working/step3") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step3"))
...
oof, pair_models, oof_fam, fam_models = fit_pair_cv(y2, w_train2, "stage-2 pair model")
...
print(json.dumps({"behaviour_rho": BEHAVIOR_RHO, "oof_labelled_pair_ap": round(labelled_ap, 4), "oof_eval_mirrored_pair_ap": round(pu_ap, 4), "oof_evidence_map5": round(oof_map5, 4),
