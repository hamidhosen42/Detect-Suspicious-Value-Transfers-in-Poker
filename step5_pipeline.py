# %% [markdown]
# # Step 5 — Other-phase pair baselines for *Detect Suspicious Value Transfers in Poker*
#
...
WORK_DIR = Path("/kaggle/working/step5") if Path("/kaggle/working").exists() else Path(os.environ.get("POKER_WORK_DIR", "./work_step5"))
...
def pool_features(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    pool = (
        ...
        .with_columns(
            ...
            (pl.col("hand_idx").cast(pl.Float32) / 3000.0).alias("idx_rel"),
...
DEV_PF = WORK_DIR / "dev_pair_features_v5.parquet"
